...
const Feed = (props) => {
    ...
    const [selectedSituationName, setSelectedSituationName] = useState({});
    ...
    const handleSituation = (event, value) => {
      if (value !== null) {
        setSelectedSituationName(value.label);
    ...
    const handleOption = (event, value) => {
      if (value !== null) {
        setselectedOptionName(value.label)
      }
    }
    ...
    const get_optionNames = (situationName) => {
      ...
      .then((response) => {
        ...
        if (response.data.state === "okay") {
          ...
          let tempOptionName = Object.keys(response.data.options).map(option => {
            return ({
              label:option
          })
          })
    ...
    const get_situationNamesfor_select = () => {
        ...
        let tempOptionName = situationName.map(situation => {
          return ({
    ...
    return (
      <StyledFeed>
        ...
        <div id="situations">
          {console.log("situation names>>>>>", situationNames)}
          <Autocomplete
              id="sitaution-select"
              sx={{ width: '100%' }}
